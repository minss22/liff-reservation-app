...
export default function SelectDatetimePage({ branchId, treatmentId, onNext, onBack }: SelectDatetimePageProps) {
  ...
  useEffect(() => {
    ...
    reservationApi.getAvailableDates(branchId, treatmentId, yearMonth)
      .then((res: any) => setAvailableDates(res))
      .catch(console.error)
      .finally(() => setIsLoadingDates(false))
  }, [yearMonth, branchId, treatmentId])

  useEffect(() => {
    if (!selectedDate) return
    setIsLoadingSlots(true)
    setSelectedTime(null)
    reservationApi.getAvailableSlots(branchId, treatmentId, selectedDate)
      .then((res: any) => setSlots(res))
      .catch(console.error)
      .finally(() => setIsLoadingSlots(false))
  }, [selectedDate, branchId, treatmentId])
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#fff', display: 'flex', flexDirection: 'column' }}>
      <TopBar title="날짜 · 시간 선택" onBack={onBack} />
      ...
      <div style={{ flex: 1, padding: '20px 20px 120px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <button onClick={() => setYearMonth(addMonth(yearMonth, -1))} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', padding: '4px 8px', color: '#333' }}>‹</button>
          <span style={{ fontSize: 16, fontWeight: 700, color: '#111' }}>{year}년 {month}월</span>
          <button onClick={() => setYearMonth(addMonth(yearMonth, 1))} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', padding: '4px 8px', color: '#333' }}>›</button>
        </div>

        {isLoadingDates ? (
          <LoadingSpinner message="날짜 조회 중..." />
        ) : (
          <div>
        ...
        {selectedDate && (
          <section>
            ...
            {isLoadingSlots ? (
              <LoadingSpinner message="시간 조회 중..." />
            ) : slots.length === 0 ? (
              <p style={{ fontSize: 14, color: '#999', textAlign: 'center', padding: 16 }}>선택 가능한 시간이 없습니다</p>
            ) : (
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, padding: '16px 20px 32px', background: '#fff', borderTop: '1px solid #F0F0F0' }}>
        <Button fullWidth disabled={!selectedDate || !selectedTime} onClick={() => selectedDate && selectedTime && onNext(selectedDate, selectedTime)}>
          예약 내용 확인
        </Button>
      </div>
    </div>
